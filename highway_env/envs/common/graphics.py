...
class EnvViewer(object):
    ...
    def set_agent_action_sequence(self, actions: List['Action']) -> None:
        ...
        if isinstance(self.env.action_type, DiscreteMetaAction):
            seq_actions = [self.env.action_type.actions[a] for a in actions]
        elif isinstance(self.env.action_type, ContinuousAction):
            for j in range(2):
                lis_actions = []
                for k in range(len(actions)):
                    lis_actions.append(actions[k][j])
                seq_actions.append(lis_actions)

        if len(seq_actions) > 1:
            for i in range(len(self.env.vehicle)):
                self.vehicle_trajectory.append(self.env.vehicle[i].predict_trajectory(seq_actions[i],
                                                                          1 / self.env.config["policy_frequency"],
                                                                          1 / 1 / self.env.config["policy_frequency"],
    ...
    def display(self) -> None:
        ...
        self.vehicle_trajectory = []
        # if self.env.trajectory is not None:
        #     self.set_agent_action_sequence(self.env.trajectory)
        #print(self.vehicle_trajectory)
        if self.vehicle_trajectory:
            for i in range(len(self.vehicle_trajectory)):
                VehicleGraphics.display_trajectory(
                    self.vehicle_trajectory[i],
                    self.sim_surface,
        ...
        #################
        try:
            for gp in self.env.ref_path:
                point = self.env.viewer.sim_surface.pos2pix(gp[0],gp[1])
                pygame.draw.circle(self.env.viewer.sim_surface,(255, 0, 0), point, 2)
        except Exception as e:
            print('Cannot display reference path')
            print(e)
            traceback.print_exc()
            print()
