...
class ContinuousAction(ActionType):
    ...
    def act(self, action: np.ndarray) -> None:
        ...
        if self.longitudinal and self.lateral:
            for i in range(len(self.controlled_vehicle)):
                self.controlled_vehicle[i].act({
                    "acceleration": utils.lmap(action[i][0], [-1, 1], self.acceleration_range),
                    "steering": utils.lmap(action[i][1], [-1, 1], self.steering_range),
                })
