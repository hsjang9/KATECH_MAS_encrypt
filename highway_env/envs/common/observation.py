...
class KinematicObservation(ObservationType):
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            ...
            self.features_range = {
                "x": [-5.0 * MDPVehicle.MAX_SPEED, 5.0 * MDPVehicle.MAX_SPEED],
                "y": [-AbstractLane.DEFAULT_WIDTH * len(side_lanes), AbstractLane.DEFAULT_WIDTH * len(side_lanes)],
                "vx": [-2*MDPVehicle.MAX_SPEED, 2*MDPVehicle.MAX_SPEED],
                "vy": [-2*MDPVehicle.MAX_SPEED, 2*MDPVehicle.MAX_SPEED]
            }
    ...
    def observe(self) -> np.ndarray:
        ...
        df = []
        obs = []
        for nagent in range(len(self.env.vehicle)):
            df.append(pd.DataFrame([]))
            obs.append([])
        for nagent in range(len(self.env.vehicle)):
            # Add ego-vehicle
            # df[nagent] = pd.concat([df[nagent], pd.DataFrame.from_records([self.env.graph[nagent][0].to_dict()])[self.features]],ignore_index=True)
            # Add nearby traffic
            origin = self.env.graph[nagent][0] if not self.absolute else None
            for obj in self.env.graph[nagent]:
                # if obj is not self.env.graph[nagent][0]:
                df[nagent] = pd.concat([df[nagent], pd.DataFrame.from_records([obj.to_dict(origin)])[self.features]], ignore_index=True)
            #df[nagent].drop_duplicates(ignore_index=True, inplace=True)

            # Normalize and clip
            if self.normalize:
                df[nagent] = self.normalize_obs(df[nagent])
            # Fill missing rows
            if df[nagent].shape[0] < self.vehicles_count:
                rows = np.zeros((self.vehicles_count - df[nagent].shape[0], len(self.features)))
                df[nagent] = pd.concat([df[nagent], pd.DataFrame(data=rows, columns=self.features)], ignore_index=True)
            # Reorder
            df[nagent] = df[nagent][self.features]
            obs[nagent] = df[nagent].values.copy()
            if self.order == "shuffled":
                self.env.np_random.shuffle(obs[nagent][1:])
        # Flatten
        obs = [data.astype(self.space().dtype) for data in obs]
        return obs
...
class OccupancyGridObservation(ObservationType):
    ...
    def __init__(self,
        ...
        self.grid_shape = np.asarray(np.floor((self.grid_size[:, 1] - self.grid_size[:, 0]) / self.grid_step),
                                dtype=np.uint8)
    ...
    def space(self) -> spaces.Space:
        if self.as_image:
            return spaces.Box(shape=self.grid[0].shape, low=0, high=255, dtype=np.uint8)
        else:
            return spaces.Box(shape=self.grid[0].shape, low=-np.inf, high=np.inf, dtype=np.float32)
    ...
    def observe(self) -> np.ndarray:
        self.grid = [np.zeros((len(self.features), *self.grid_shape)) for _ in range(len(self.env.vehicle))]
        self.observer_vehicle = [v for v in self.env.vehicle]
        df = []
        if not self.env.road:
            return np.zeros(self.space().shape)
        # Initialize empty data
        for nagent in range(len(self.observer_vehicle)):
            self.grid[nagent].fill(np.nan)
        ...
        if self.absolute:
            ...
            for nagent in range(len(self.observer_vehicle)):
                ...
                df.append(pd.DataFrame.from_records([self.observer_vehicle[nagent].to_dict()]))
                for v in self.env.road.vehicles:
                    if v is not self.observer_vehicle[nagent]:
                        df[nagent] = pd.concat([df[nagent], pd.DataFrame.from_records([v.to_dict(self.observer_vehicle[nagent])])], ignore_index=True)
                
                # Normalize
                df[nagent] = self.normalize(df[nagent])
                # Fill-in features
                for layer, feature in enumerate(self.features):
                    if feature in df[nagent].columns:  # A vehicle feature
                        for value, vehicle in df[nagent][::-1].iterrows():
                            if value == 0:
                                self.grid[nagent][layer, 3, 1] = vehicle[feature] # agent 데이터
                            else:
                                ...
                                cell = self.pos_to_index((x, y), self.observer_vehicle[nagent], relative=not self.absolute)

                                if 0 <= cell[1] < self.grid[nagent].shape[-1] and 0 <= cell[0] < self.grid[nagent].shape[-2]:
                                    self.grid[nagent][layer, -cell[0]-1, cell[1]] = vehicle[feature]
            ...
            if self.clip:
                obs1 = np.clip(obs1, -1, 1)
                obs2 = np.clip(obs2, -1, 1)

            if self.as_image:
                obs1 = ((np.clip(obs1, -1, 1) + 1) / 2 * 255).astype(np.uint8)
                obs2 = ((np.clip(obs2, -1, 1) + 1) / 2 * 255).astype(np.uint8)

            obs1 = np.nan_to_num(obs1).astype(self.space().dtype)
            obs2 = np.nan_to_num(obs2).astype(self.space().dtype)
            
            for i in range(len(obs1)):
                trans_obs1.append(obs1[i].reshape(-1,1))
                trans_obs2.append(obs2[i].reshape(-1,1))
            trans_obs1 = np.array(np.hstack(trans_obs1)).squeeze()
            trans_obs2 = np.array(np.hstack(trans_obs2)).squeeze()

            return trans_obs1, trans_obs2

    def pos_to_index(self, position: Vector, observer, relative: bool = False) -> Tuple[int, int]:
        """
