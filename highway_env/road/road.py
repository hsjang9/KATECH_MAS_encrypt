...
class RoadNetwork(object):
    ...
    def get_lane(self, index: LaneIndex) -> AbstractLane:
        ...
        if _id is None and len(self.graph[_from][_to]) == 1:
            _id = 0
        if _id is None and len(self.graph[_from][_to]) == 2:
            _id = 1
    ...
    def next_lane(self, current_index: LaneIndex, vehicle = None, route: Route = None, position: np.ndarray = None,
        ...
        if route:
            if route[0][:2] == current_index[:2]:  # We just finished the first step of the route, drop it.
                vehicle.past_route.append(route[0])
                route.pop(0)
    ...
    def rear_lane_index(self, past_lane_index: LaneIndex, lane_index: LaneIndex) -> List[LaneIndex]:

        _pastfrom, _pastto, _pastid = past_lane_index
        _from, _to, _id = lane_index

        return _pastfrom, _pastto, _id 
...
class Road(object):
    ...
    def neighbour_vehicles(self, vehicle: 'kinematics.Vehicle', lane_index: LaneIndex = None) \
        ...
        # Find neighbours in the current lane
        v_front, v_rear, s_front, s_rear = self.find_neighbours(lane_index, vehicle)

        # Add next lane to the consideration
        next_lane_index = self.network.next_lane(lane_index, position=vehicle.position)
        if next_lane_index:
            v_front_next, v_rear_next, s_front_next, s_rear_next = self.find_neighbours(next_lane_index, vehicle)

            # Compare and choose the closer vehicles
            if v_front_next and (not v_front):
                v_front = v_front_next

        if len(vehicle.past_route) >= 1:
            past_lane_index = vehicle.past_route[-1]
            rear_lane_index = self.network.rear_lane_index(past_lane_index, lane_index)
            v_front_past, v_rear_past, s_front_past, s_rear_past = self.find_neighbours(rear_lane_index, vehicle)

            # Compare and choose the closer vehicles
            if v_rear_past and (not v_rear):
                v_rear = v_rear_past

        return v_front, v_rear
    
    def find_neighbours(self, lane_index, vehicle):
        ...
        for v in self.vehicles + self.objects:
            if v is not vehicle and not isinstance(v, Landmark):
                ...
                if s_v < s and (s_rear is None or s_v > s_rear):
                    s_rear = s_v
                    v_rear = v
        return v_front, v_rear, s_front, s_rear
