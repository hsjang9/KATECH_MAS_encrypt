...
class ControlledVehicle(Vehicle):
    ...
    def plan_route_to(self, destination: str) -> "ControlledVehicle":
        ...
        if path:
            ...
            self.route = [self.lane_index] + [(path[i], path[i + 1], _id) for i in range(len(path) - 1)]
        else:
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        if action is not None:
            self.target_speed += action["acceleration"]
            if self.target_speed < 0:
                self.target_speed = 0
            if self.target_speed > 20:
                self.target_speed = 20

        action = {"acceleration": self.speed_control(self.target_speed),
                  "steering": self.waypoint_control()}
    ...
    def waypoint_control(self) -> float:
        """
        pure persuit
        """
        wp_list = self.global_path
        waypoint, lf = self.select_waypoint(wp_list)
        self.lookahead_point = waypoint
        theta = math.atan2(waypoint[1] - self.position[1], waypoint[0] - self.position[0]) - self.heading
        steering_angle = math.atan2(2 * self.LENGTH * math.sin(theta), lf)        
        
        return float(steering_angle)
    
    def select_waypoint(self, wp_list):
        rear_x = self.position[0] - ((self.LENGTH / 2) * math.cos(self.heading))
        rear_y = self.position[1] - ((self.LENGTH / 2) * math.sin(self.heading))
        if self.nearest_idx is None:
            dx = [rear_x - wp_list[i][0] for i in range(len(wp_list))]
            dy = [rear_y - wp_list[i][1] for i in range(len(wp_list))]
            d = np.hypot(dx, dy)
            ind = np.argmin(d)
            self.nearest_idx = ind
        else:
            ind = self.nearest_idx
            distance_this_index = math.hypot(rear_x - wp_list[ind][0], rear_y - wp_list[ind][1])

            while True:
                distance_next_index = math.hypot(rear_x - wp_list[ind+1][0], rear_y - wp_list[ind+1][1])
                if distance_this_index < distance_next_index:
                    break
                ind = ind + 1 if (ind + 1) < len(wp_list) else ind
                distance_this_index = distance_next_index
            self.nearest_idx = ind

        lf = 0.1 * self.speed + 5
        
        while lf > math.hypot(rear_x - wp_list[ind][0], rear_y - wp_list[ind][1]):
            if (ind + 1) >= len(wp_list):
                break
            ind += 1
        return wp_list[ind], lf
    ...
    def predict_trajectory(self, actions: List, action_duration: float, trajectory_timestep: float, dt: float) \
        ...
        for action in actions:
            action_dict = {'acceleration': action[0],
                           'steering': action[1]}
            v.act(action_dict)  # High-level decision
